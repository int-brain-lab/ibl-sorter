...
def test_destripe_params_defaults():
    p = DestripeParams()
    assert p.k_filter is True  # k-filter is the default
    assert p.k_kwargs == {'ntr_pad': 60, 'ntr_tap': 0, 'lagc': 3000, 'butter_kwargs': {'N': 3, 'Wn': 0.01, 'btype': 'highpass'}, 'epsilon': None}
